...
impl<T> TypedArena<T> {
    ...
    pub fn new() -> TypedArena<T> {
        TypedArena {
            lock: MTLock::new(TypedArenaInner {
    ...
    pub fn alloc(&self, object: T) -> &mut T {
        ...
        unsafe {
            if mem::size_of::<T>() == 0 {
                this.ptr
                    .set(intrinsics::arith_offset(this.ptr.get() as *mut u8, 1)
                        as *mut T);
...
pub struct DroplessArena {
    lock: MTLock<DroplessArenaInner>,
}
...
impl DroplessArena {
    pub fn new() -> DroplessArena {
        DroplessArena {
            lock: MTLock::new(DroplessArenaInner {
    ...
    pub fn alloc<T>(&self, object: T) -> &mut T {
        unsafe {
            ...
            this.ptr.set(
                intrinsics::arith_offset(this.ptr.get(), mem::size_of::<T>() as isize) as *mut u8,
            );
            // Write into uninitialized memory.
...
mod tests {
    ...
    pub fn test_unused() {
        ...
        let lock = arena.lock.lock();
        assert!(lock.chunks.borrow().is_empty());
    }
