     ...
     $($(#[$attr:meta])*
       [$($modifiers:tt)*] fn $name:ident: $node:ident($K:ty) -> $V:ty,)*) => {
        ...
        impl<$tcx> Maps<$tcx> {
            ...
            fn set_cycle_error(&self, query: Query<$tcx>, error: CycleError<$tcx>) {
                match query {
                    $(Query::$name(key) => {
                        self.$name.lock().map.insert(key, QueryResult::Cycle(error));
                    })*
                }
            }
        ...
        impl<'a, $tcx, 'lcx> queries::$name<$tcx> {
                                      ...
                                      Result<($V, DepNodeIndex), CycleError<$tcx>>>
            {
                let lock = loop {
                    ...
                    let job = if let Some(value) = lock.map.get(key) {
                        match *value {
                            QueryResult::Cycle(ref error) => return Err(Err(error.clone())),
                            ...
                            -> Result<$V, CycleError<$tcx>>
            {
                ...
                macro_rules! get_lock {
                    () => {{
                        match Self::try_get_lock(tcx, &key) {
                            Ok(lock) => lock,
                            Err(result) => {
...
*/
                ...
                let (r, job) = ty::tls::with_related_context(tcx, move |icx| {
                    let layout_depth = if stringify!($name) == "layout_raw" {
                        icx.query.layout_depth + 1
                    } else {
                        icx.query.layout_depth
                    };

                    let job = Lrc::new(QueryJob::new(Some(entry),
                                                     true,
                                                     layout_depth,
                                                     Some(ty::maps::job::Ref(&*icx.query))));
                    job.start();
                    ...
                    let r = {
                        ...
                        let icx = ty::tls::ImplicitCtxt {
                            ...
                            query: job.clone(),
                            task: icx.task,
                            buffer_diagnostics: icx.buffer_diagnostics,
                        };
            ...
            fn force(tcx: TyCtxt<'a, $tcx, 'lcx>,
                ...
                let lock = match Self::try_get_lock(tcx, &key) {
                    Ok(lock) => lock,
